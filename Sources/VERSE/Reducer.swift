...
public struct Reducer<State, Action, Environment> {
    ...
    ///     }
    ///
    /// - Parameter reducer: a function signature that takes state, action and environment
    ...
    /// Combines current reducer with another reducer into a single one by running
    /// each one on state in order, and merging all of the effects
    ///
    /// It is important to note that the order of combining reducers matter. Combining `reducerA` with
    /// `reducerB` is not necessarily the same as combining `reducerB` with `reducerA`.
    ///
    ...
    public func pullback<GlobalState, GlobalAction, GlobalEnvironment>(
        ...
        .init { globalState, globalAction, globalEnvironment in
            guard let localAction = toLocalAction.extract(from: globalAction) else {
                return .none
            }
            return self.reducer(
    ...
    ///   - toLocalState: a key path that can get/set `State` inside `GlobalState`
    ///   - toLocalAction: a case path that can extract/embed `Action` from `GlobalAction`
    ///   - localEnvironment: a function that returns local `Environment` object
    /// - Returns: a reducer that works on `GlobalState`, `GlobalAction`, `GlobalEnvironment`
    public func pullback<GlobalState, GlobalAction, GlobalEnvironment>(
        state toLocalState: WritableKeyPath<GlobalState, State>,
        action toLocalAction: EnumKeyPath<GlobalAction, Action>,
        environment localEnvironment: @escaping @autoclosure () -> Environment
    ) -> Reducer<GlobalState, GlobalAction, GlobalEnvironment> {
        .init { globalState, globalAction, globalEnvironment in
            guard let localAction = toLocalAction.extract(from: globalAction) else {
                return .none
    ...
    ///             environment: {
    ///                 ModalEnvironment(mainQueue: $0.mainQueue)
    ///             }
    ///         ),
    ///         Reducer { state, action, environment in
    ///             ...
    ...
    ///             Button("Child Action") {
    ///                 parentViewStore.send(.child(.action))
    ///             }
    ///             ...
    ///         }
    ///
    ...
    public func forEach<GlobalState, GlobalAction, GlobalEnvironment>(
        state toLocalState: WritableKeyPath<GlobalState, [State]>,
        action toLocalAction: EnumKeyPath<GlobalAction, (Int, Action)>,
        environment toLocalEnvironment: @escaping (GlobalEnvironment) -> Environment,
        breakpointOnNil: Bool = true,
        _ file: StaticString = #file,
        _ line: UInt = #line
    ...
    public func forEach<GlobalState, GlobalAction, GlobalEnvironment, ID>(
        ...
        action toLocalAction: EnumKeyPath<GlobalAction, (ID, Action)>,
        environment toLocalEnvironment: @escaping (GlobalEnvironment) -> Environment,
        breakpointOnNil: Bool = true,
        _ file: StaticString = #file,
        _ line: UInt = #line
    ...
    public func forEach<GlobalState, GlobalAction, GlobalEnvironment, Key>(
        ...
        action toLocalAction: EnumKeyPath<GlobalAction, (Key, Action)>,
        environment toLocalEnvironment: @escaping (GlobalEnvironment) -> Environment,
        breakpointOnNil: Bool = true,
        _ file: StaticString = #file,
        _ line: UInt = #line
    ...
    public func pullbackWithShared<SharedState, LocalPrivateState, GlobalPrivateState, GlobalAction, GlobalEnvironment>(
    ...
    public func optionalWithShared<SharedState, PrivateState>(
        ...
        line: UInt = #line
    ) -> Reducer<CombinedState<SharedState, PrivateState?>, Action, Environment>
...
extension Reducer {
    ...
    public func withContext<Context, PrivateState>(contextHandle: ContextHandle<Context>) -> Reducer<PrivateState, Action, Environment>
