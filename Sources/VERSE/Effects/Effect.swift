...
public struct Effect<Output, Failure: Error>: Publisher {
    ...
    public func map<T>(_ transform: @escaping (Output) -> T) -> Effect<T, Failure> {
        .init(map(transform) as Publishers.Map<Self, T>)
    }
    ...
    ///   VERSE in functions like `Reducer.combine`
    ///
