...
extension Effect where Failure == Never {
    ...
    /// to create a timer in the VERSE, it is not advisable. This API only allows
    ...
    ///
    /// In the VERSE we test time-based effects like this by using the
    ...
    ///         case .startButtonTapped:
    ///             return Effect
    ///                 .timer(id: TimerId(), every: 1, on: env.mainQueue)
    ...
    ///     }
    ///
    /// - Note: This effect is only meant to be used with features built in the VERSE,
    ///   and returned from a reducer. If you want a testable alternative to
    ///   Foundation's `Timer.publish` you can use the publisher `Publishers.Timer` that is included
    ///   in this library via the
    ///   [`CombineSchedulers`](https://github.com/Incetro/combine-schedulers) module.
    ///
    /// - Parameters:
