...
/// dismissal of sheets, and that does not play nicely with the VERSE. The library
...
/// to present the action sheet in a way that works best with the VERSE:
///
...
public struct ActionSheetState<Action> {
    ...
    public private(set) var message: TextState?

    /// Action sheet title
    public private(set) var title: TextState
...
extension ActionSheetState: Equatable where Action: Equatable {
    ...
    public static func == (lhs: Self, rhs: Self) -> Bool {
        lhs.title == rhs.title && lhs.message == rhs.message && lhs.buttons == rhs.buttons
    }
}
...
extension View {
    ...
    public func actionSheet<Action>(
        ...
        WithViewStore(store, removeDuplicates: { $0?.id == $1?.id }) { viewStore in
            actionSheet(item: viewStore.binding(send: dismiss)) { state in
