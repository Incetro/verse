...
public struct SwitchStore<State, Action, Content>: View where Content: View {
    ...
    /// Default initializer
    /// - Parameters:
    ///   - store: target store instance
    ///   - content: view's content builder
    init(
    ...
    public var body: some View {
        content().environmentObject(StoreObservableObject(store: store))
    }
}

/// A view that handles a specific case of enum state in a ``SwitchStore``.
public struct CaseLet<GlobalState, GlobalAction, LocalState, LocalAction, Content>: View where Content: View {

    // MARK: - Properties

    /// Store observable object instance
    @EnvironmentObject
    private var store: StoreObservableObject<GlobalState, GlobalAction>

    /// A function that can extract a case of switch store state,
    /// which can be specified using case path literal syntax, _e.g._ `/State.case`
    ...
    /// A function that is given a store of the given case's state and returns a view
...
extension SwitchStore {
    ...
    Content == WithViewStore<
        ...
        _ConditionalContent<
            CaseLet<State, Action, State1, Action1, Content1>,
            Default<ExhaustivityCheckView<State, Action>>
    >
    >
    {
        self.init(store) {
            content()
            Default { ExhaustivityCheckView<State, Action>(file: file, line: line) }
        }
    }
...
// MARK: - ExhaustivityCheckView

public struct ExhaustivityCheckView<State, Action>: View {
    ...
    public var body: some View {
            ...
            "\(debugCaseOutput(store.wrappedValue.state.value))" was encountered by a \
