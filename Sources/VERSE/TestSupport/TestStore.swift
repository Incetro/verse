...
/// VERSE. It allows you to send a sequence of actions to the store, and each
...
///             request: { _ in Effect(value: ["VERSE"]) }
///         )
///     )
///
///     // Change the query
...
public final class TestStore<State, LocalState, Action: Equatable, LocalAction, Environment> {
    ...
    /// Default initializer
    /// - Parameters:
    ///   - environment: current store environment
    ///   - file: target file path
    ///   - fromLocalAction: local action transformer
    ///   - initialState: initial state instance
    ///   - line: code line value
    ///   - reducer: current reducer instance
    ///   - toLocalState: local state transformer
    private init(
