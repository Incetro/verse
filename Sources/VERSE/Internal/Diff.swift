...
func diff(_ first: String, _ second: String) -> String? {
    ...
    /// Difference helper
    /// - Parameters:
    ///   - first: first string slice
    ///   - second: second string slice
    /// - Returns: Difference array
    func diffHelp(
        _ first: ArraySlice<Substring>,
        _ second: ArraySlice<Substring>
    ) -> [Difference] {
        ...
        if count == 0 {
            ...
            differences.append(contentsOf: diffHelp(first.suffix(from: firstIndex + count), second.suffix(from: secondIndex + count)))
            return differences
        }
    }
    ...
    if differences.count == 1, case .both = differences[0].which {
        return nil
    }
